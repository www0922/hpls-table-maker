...
description: 统一生成 HPLS 项目的 PE100、PE150 和 XP 文库表。当用户说“帮我做 PE100 表”“生成 PE150 pooling 表”“做 XP 表”“处理某个上机表”，或提供相关 Excel 源表并要求生成文库 pooling/PCR 定量表时使用。识别目标表型，校验本批次输入文件，调用对应项目的 run_all.py 全流程，并返回生成的 Excel 文件。
agent_created: true
---
...
本 Skill 是 HPLS 项目的包装层，依赖项目代码（`PE_100/`、`PE_150/`、`XP/`、`common/`）才能运行。Skill 通过自身在项目内的位置自动探测根目录，因此：
...
将 PE100、PE150、XP 三套现有自动化代码作为一个统一能力使用。根据用户话语识别表型，调用对应入口，完成源表校验、表格生成、输出校验和结果交付。
...
```bash
python scripts/run_table.py --type <pe100|pe150|xp> --check
```
...
```bash
python scripts/run_table.py --type <pe100|pe150|xp>
```
