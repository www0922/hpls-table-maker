"""
XP项目配置文件: 自动发现源表、管理路径
=====================================
目录结构:
  input_data/               : 放源数据文件
  output_result/            : 输出目录
  XP_pooling表模板.xlsx     : Pooling输出模板(只读)
  XP_PCR定量表模板.xlsx     : PCR定量表模板(只读)
"""
import os
import shutil
...
TEMPLATE_POOL = ROOT / 'XP_pooling表模板.xlsx'
TEMPLATE_PCR = ROOT / 'XP_PCR定量表模板.xlsx'

from datetime import datetime as _dt
_date_str = _dt.now().strftime('%Y%m%d')
DST_POOL = str(OUTPUT_DIR / f'{_date_str}文库pooling表.xlsx')
...
def find_file(keyword):
    ...
    """
    matches = sorted(INPUT_DATA.glob(f'*{keyword}*.xlsx'), key=lambda path: path.name)
    if not matches:
...
def get_src_a():
    """A表: 上机表"""
    return find_file('Xplus上机')


def get_target_sheets():
    """根据A表动态获取目标数据sheet列表, 如['A']或['A','B']"""
    import openpyxl
